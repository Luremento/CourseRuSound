        ...
        @if ($type != 'albom')
            <div
                ...
                <button id="play-stop-button-{{ $track->id }}"
                        class="hover:scale-110 text-white opacity-0 transform translate-y-3 group-hover:translate-y-0 group-hover:opacity-100 transition z-10"
                        onclick="event.preventDefault(); togglePlayStop('{{ asset('storage/' . $track->music_file) }}', {{ $track->id }}, '{{ $track->name }}', '{{ $artist->name }}', '{{ asset('storage/' . $track->cover_file) }}')">
                    <svg id="play-icon-{{ $track->id }}" xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" class="bi bi-play-circle-fill" viewBox="0 0 16 16">
                        <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM6.79 5.093A.5.5 0 0 0 6 5.5v5a.5.5 0 0 0 .79.407l3.5-2.5a.5.5 0 0 0 0-.814l-3.5-2.5z" />
                    </svg>
                    <svg id="stop-icon-{{ $track->id }}" style="display: none;" xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" class="bi bi-stop-circle-fill" viewBox="0 0 16 16">
...
<script>
    ...
    function togglePlayStop(trackUrl, trackId, trackName, artistName, trackPhoto) {
        ...
        if (audio.paused) {
            // Если аудио остановлено, начинаем воспроизведение нового трека
            source.src = trackUrl;
            audio.load();
            audio.play();
            ...
            // Обновляем информацию о текущем треке на странице
            ...
            // Удаляем информацию о треке из localStorage
            localStorage.removeItem('currentTrack');

            // Очищаем информацию о текущем треке на странице
            updateTrackInfo('', '');
        }
    }

    function updateTrackInfo(trackName, artistName) {
        var trackInfoContainer = document.getElementById('current-track-info');
        if (trackName && artistName) {
            trackInfoContainer.innerHTML = `
            <h2 class="text-2xl font-semibold">${trackName}</h2>
